...
const Comment = ({ comment, onUpdateComment, onReply }) => {
  const [showReplies, setShowReplies] = useState(false);
  return (
    <li className="list-none ">
      <article
        className="px-4 py-4 sm:px-6 rounded-lg bg-white shadow"
        aria-labelledby="question-title-41150 "
      >
        <div>
      ...
      {showReplies ? (
        <div className="ml-20 md:ml-32">
          <AddReplyInput comment={comment} onSubmit={onReply} />
...
const mapDispatchToProps = (dispatch) => ({
  onReply: (comment) => dispatch(updateCommentRequest(comment)),
});
